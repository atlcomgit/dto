...
\
@method public static **[toArrayBlank](#пример-40)**()\
*Возвращает массив с пустыми значениями всех свойств dto.*\
\
@method public static **[toArrayBlankRecursive](#пример-40)**()\
...
@override @method protected **[defaults](#пример-05)**(): array { return []; }\
*Задаёт массив значений для свойств при заполнении Dto по умолчанию.*\
\
@override @method protected **[mappings](#пример-06)**(): array { return []; }\
*Задаёт массив имён свойств для маппинга в другие свойства.*\
\
@override @method protected **[casts](#пример-08)**(): array { return []; }\
*Задаёт массив приведения типов свойств при заполнении Dto.*\
\
@override @method protected **[exceptions](#пример-33)**(string $messageCode, array $messageItems): string {}\
*Возвращает сообщение об ошибке по его коду при работе с Dto.*\
\
@override @method protected **[onCreating](#пример-44)**(array &$data): void {}\
*Метод-хук вызывается перед созданием и заполнением Dto.*\
\
@override @method protected **[onCreated](#пример-44)**(array $data): void {}\
*Метод-хук вызывается после создания и заполнения Dto.*\
\
@override @method protected **[onFilling](#пример-11)**(array &$array): void {}\
*Метод-хук вызывается перед заполнением Dto.*\
\
@override @method protected **[onFilled](#пример-12)**(array $array): void {}\
*Метод-хук вызывается после заполнения Dto.*\
\
@override @method protected **[onMerging](#пример-13)**(array &$array): void {}\
*Метод-хук вызывается перед объединением массива в Dto.*\
\
@override @method protected **[onMerged](#пример-14)**(array $array): void {}\
*Метод-хук вызывается после объединения массива в Dto.*\
\
@override @method protected **[onSerializing](#пример-15)**(array &$array): void {}\
*Метод-хук вызывается перед сериализацией в массив.*\
\
@override-хук @method protected **[onAssigning](#пример-32)**(string $key, mixed $value): void {}\
*Выполняется перед изменением значения свойства Dto.*\
\
@override-хук @method protected **[onAssigned](#пример-32)**(string $key): void {}\
...
\
@override-хук @method protected **[onException](#пример-32)**(Throwable $exception): void {}\
...
\
@method public **[withCustomOptions](#пример-43)**()\
...
\
@method public **[setCustomOption](#пример-42)**()\
*Добавляет свою опцию в dto.*\
\
@method public **[getCustomOption](#пример-42)**()\
